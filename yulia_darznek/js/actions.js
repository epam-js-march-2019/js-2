...
var itemTemplate = document.querySelector('#order-item').content.querySelector('.order-item');
...
function setValuesInCard(parentElm, valuePrice, valueCalories) {
  parentElm.querySelector('.menu__card-sum').textContent = valuePrice + PRICE_UNIT;
  parentElm.querySelector('.menu__card-calories').textContent = valueCalories + CALORIES_UNIT;
}

function updateOrderTotalValues() {
  document.querySelector('.order__total-calories').textContent = order.calculateCalories() + CALORIES_UNIT;
  document.querySelector('.order__total-sum').textContent = order.calculateTotal() + PRICE_UNIT;
}
...
function addItemToOrder(parentElm, elm) {
...
function setParams(elm, data) {
  ...
  elm.querySelector('.order__item-value_calories').textContent = data.caloriesTotal + CALORIES_UNIT;
  elm.querySelector('.order__item-value_price').textContent = data.priceTotal + PRICE_UNIT;
